...
import { Flame, TrendingUp, Clock, TrendingDown, Loader2, Database } from 'lucide-react';
...
export default function TrendForecaster() {
  ...
  // Primary query: Read existing data from database (cache-first, 24h staleTime)
  const { data: forecast, isLoading } = useQuery({
    queryKey: ['/api/trend-history', 'forecaster', selectedNiche],
    queryFn: async () => {
      console.log(`💾 TrendForecaster: Reading ${selectedNiche} trends from cache...`);
    ...
    staleTime: 1000 * 60 * 60 * 24, // 24 hours - data updates daily at midnight UTC
  ...
  // Format last update timestamp from database
  const formatLastUpdate = () => {
    ...
    try {
      const lastUpdateDate = new Date(databaseTimestamp);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              TikTok trends powered by Perplexity AI - Updated daily at midnight UTC
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="text-right">
              ...
              <p className="text-xs font-semibold text-gray-700">{formatLastUpdate()}</p>
            </div>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        ...
        <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-xs text-blue-800">
            💡 <strong>Automated Updates:</strong> Trends are automatically fetched daily at midnight UTC. Data is served from cache for optimal performance.
          </p>
        </div>
      </CardContent>
    </Card>
