...
// Database file path - ES module compatible
...
class FeedbackLogger {
  ...
  async getMostSuccessfulPatterns(): Promise<{
    ...
    return new Promise((resolve, reject) => {
      ...
      // Execute tone query
      this.db.get(toneQuery, (err, row: any) => {
        if (err) {
      ...
      this.db.get(templateQuery, (err, row: any) => {
        if (err) {
...
export const getMostSuccessfulPatterns = async (): Promise<{
  ...
  return await feedbackLogger.getMostSuccessfulPatterns();
};
