...
export async function fetchTrendingTravelProducts(): Promise<any[]> {
  ...
  const prompt = `You are a product research API. Return 3 trending travel products from Amazon that are viral on TikTok or Instagram as of ${currentMonth} ${currentYear}. Respond ONLY with a JSON array and nothing else. Follow this structure exactly:
...
[
  { "product": "Compression Packing Cubes 6-Set", "brand": "BAGSMART", "mentions": 1340000, "reason": "Space saving packing viral hack" },
  { "product": "Portable Phone Charger 20000mAh", "brand": "Anker", "mentions": 1100000, "reason": "Essential travel power bank" },
  { "product": "Travel Neck Pillow Memory Foam", "brand": "Cabeau", "mentions": 780000, "reason": "Comfort travel accessory trend" }
]

Requirements:
- Real travel products only (luggage, accessories, tech, organizers)
- Specific brand names (Samsonite, Away, Anker, BAGSMART, etc.)
...
JSON array only:`;
  ...
  try {
    ...
    console.log(`✅ Travel fetcher: Found ${validProducts.length} valid products`);
