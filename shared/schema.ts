...
// ===================================
// BRAND BUILDER INFRASTRUCTURE
// ===================================

// Brand development projects tracking
export const brandProjects = pgTable("brand_projects", {
  id: serial("id").primaryKey(),
  projectName: text("project_name").notNull(),
  description: text("description"),
  niche: text("niche").notNull(), // beauty, tech, fashion, fitness, food, travel, pets
  status: text("status").notNull().default("discovery"), // discovery, validation, development, pre_launch, launched, paused
  
  // Project metadata
  targetMarket: text("target_market"), // e.g., "millennials_beauty_conscious"
  businessModel: text("business_model"), // b2c, b2b, subscription, marketplace
  projectGoals: text("project_goals").array().default([]),
  tags: text("tags").array().default([]),
  
  // Key metrics and targets
  targetRevenue: decimal("target_revenue"),
  estimatedLaunchDate: timestamp("estimated_launch_date"),
  budgetAllocated: decimal("budget_allocated"),
  
  // Collaboration
  ownerId: integer("owner_id").references(() => users.id, { onDelete: 'set null' }),
  teamMembers: jsonb("team_members"), // Array of user IDs and roles
  ...
  return {
    statusIdx: index("brand_projects_status_idx").on(table.status),
    nicheIdx: index("brand_projects_niche_idx").on(table.niche),
    ownerIdx: index("brand_projects_owner_idx").on(table.ownerId),
    launchDateIdx: index("brand_projects_launch_date_idx").on(table.estimatedLaunchDate),
  };
});
...
export const marketResearch = pgTable("market_research", {
  id: serial("id").primaryKey(),
  brandProjectId: integer("brand_project_id").references(() => brandProjects.id, { onDelete: 'cascade' }),
  
  // Research identification
  researchType: text("research_type").notNull(), // market_size, competitor_analysis, trend_analysis, customer_survey
  researchTitle: text("research_title").notNull(),
  
  // Research data
  dataSource: text("data_source").notNull(), // perplexity, manual, survey, web_scraping
  rawData: jsonb("raw_data"), // Original research data
  findings: jsonb("findings"), // Processed insights
  
  // Market insights
  marketSize: decimal("market_size"), // Total addressable market
  growthRate: decimal("growth_rate"), // Annual growth percentage
  ...
  mainCompetitors: jsonb("main_competitors"), // Array of competitor objects
  marketGaps: text("market_gaps").array().default([]),
  differentiationOpportunities: text("differentiation_opportunities").array().default([]),
  
  // Research metadata
  confidenceScore: decimal("confidence_score"), // 0-1 confidence in research quality
  researchDate: timestamp("research_date").defaultNow().notNull(),
  expiryDate: timestamp("expiry_date"), // When research becomes outdated
  ...
  return {
    projectIdx: index("market_research_project_idx").on(table.brandProjectId),
    typeIdx: index("market_research_type_idx").on(table.researchType),
    sourceIdx: index("market_research_source_idx").on(table.dataSource),
    dateIdx: index("market_research_date_idx").on(table.researchDate),
    confidenceIdx: index("market_research_confidence_idx").on(table.confidenceScore),
  };
});

// Demand validation testing and results
export const demandValidation = pgTable("demand_validation", {
  id: serial("id").primaryKey(),
  brandProjectId: integer("brand_project_id").references(() => brandProjects.id, { onDelete: 'cascade' }),
  
  // Test identification
  testName: text("test_name").notNull(),
  testType: text("test_type").notNull(), // landing_page, survey, mvp_test, preorder_campaign, social_media_test
  status: text("status").notNull().default("planned"), // planned, running, completed, failed
  
  // Test configuration
  targetAudience: jsonb("target_audience"), // Demographics and psychographics
  testDuration: integer("test_duration"), // Duration in days
  sampleSize: integer("sample_size"),
  budget: decimal("budget"),
  
  // Test metrics and goals
  successCriteria: jsonb("success_criteria"), // What defines success
  keyMetrics: text("key_metrics").array().default([]), // clicks, conversions, signups, etc.
  
  // Results data
  participants: integer("participants").default(0),
  conversions: integer("conversions").default(0),
  conversionRate: decimal("conversion_rate").default("0"),
  costPerAcquisition: decimal("cost_per_acquisition"),
  
  // Detailed results
  results: jsonb("results"), // Detailed test results
  insights: text("insights").array().default([]),
  userFeedback: jsonb("user_feedback"), // Qualitative feedback
  
  // Validation scoring
  demandScore: decimal("demand_score"), // Overall demand score (0-100)
  confidenceLevel: decimal("confidence_level"), // Statistical confidence
  recommendation: text("recommendation"), // proceed, pivot, stop
  ...
  return {
    projectIdx: index("demand_validation_project_idx").on(table.brandProjectId),
    typeIdx: index("demand_validation_type_idx").on(table.testType),
    statusIdx: index("demand_validation_status_idx").on(table.status),
    demandScoreIdx: index("demand_validation_demand_score_idx").on(table.demandScore),
    dateIdx: index("demand_validation_date_idx").on(table.startDate),
  };
});
...
export const competitorAnalysis = pgTable("competitor_analysis", {
  ...
  fundingStage: text("funding_stage"), // bootstrapped, seed, series_a, etc.
  marketShare: decimal("market_share"),
  
  // Product analysis
  productOffering: jsonb("product_offering"), // Products/services offered
  pricingStrategy: jsonb("pricing_strategy"), // Pricing model and tiers
  valueProposition: text("value_proposition"),
  
  // Performance metrics
  websiteTraffic: integer("website_traffic"), // Monthly visitors
  socialMediaFollowing: jsonb("social_media_following"), // Platform-specific followers
  customerReviews: jsonb("customer_reviews"), // Review scores and sentiment
  ...
  // Strategic insights
  differentiationGaps: text("differentiation_gaps").array().default([]),
  competitiveAdvantages: text("competitive_advantages").array().default([]),
  threatLevel: text("threat_level").default("medium"), // low, medium, high, critical
  
  // Analysis metadata
  dataSource: text("data_source").notNull().default("manual"), // manual, web_scraping, api, third_party
  lastUpdated: timestamp("last_updated").defaultNow().notNull(),
  analysisConfidence: decimal("analysis_confidence"), // 0-1 confidence score
  
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (table) => {
  return {
    projectIdx: index("competitor_analysis_project_idx").on(table.brandProjectId),
    typeIdx: index("competitor_analysis_type_idx").on(table.competitorType),
    threatIdx: index("competitor_analysis_threat_idx").on(table.threatLevel),
    updateIdx: index("competitor_analysis_update_idx").on(table.lastUpdated),
  };
});

// Product opportunities and market gaps
export const productOpportunities = pgTable("product_opportunities", {
  ...
  description: text("description").notNull(),
  category: text("category").notNull(), // market_gap, trend_opportunity, competitor_weakness, customer_need
  
  // Market analysis
  marketSize: decimal("market_size"), // Estimated market size
  demandLevel: text("demand_level").default("medium"), // low, medium, high, very_high
  competitionLevel: text("competition_level").default("medium"), // low, medium, high, saturated
  
  // Opportunity scoring
  opportunityScore: decimal("opportunity_score"), // Overall score (0-100)
  feasibilityScore: decimal("feasibility_score"), // How feasible to execute (0-100)
  profitabilityScore: decimal("profitability_score"), // Expected profitability (0-100)
  riskScore: decimal("risk_score"), // Risk assessment (0-100, lower is better)
  
  // Strategic insights
  keyInsights: text("key_insights").array().default([]),
  successFactors: text("success_factors").array().default([]),
  potentialChallenges: text("potential_challenges").array().default([]),
  
  // Implementation details
  estimatedTimeToMarket: integer("estimated_time_to_market"), // Months
  estimatedInvestment: decimal("estimated_investment"),
  requiredResources: text("required_resources").array().default([]),
  
  // Supporting data
  dataSource: text("data_source").notNull(), // research, trend_analysis, customer_feedback, competitor_analysis
  supportingEvidence: jsonb("supporting_evidence"), // Links, data, research backing the opportunity
  
  // Priority and status
  priority: text("priority").default("medium"), // low, medium, high, critical
  status: text("status").default("identified"), // identified, researching, validated, rejected, pursuing
  
  // Timeline
  discoveredDate: timestamp("discovered_date").defaultNow().notNull(),
  ...
  return {
    projectIdx: index("product_opportunities_project_idx").on(table.brandProjectId),
    categoryIdx: index("product_opportunities_category_idx").on(table.category),
    scoreIdx: index("product_opportunities_score_idx").on(table.opportunityScore),
    priorityIdx: index("product_opportunities_priority_idx").on(table.priority),
    statusIdx: index("product_opportunities_status_idx").on(table.status),
  };
});

// Zod schemas for Brand Builder tables
export const insertBrandProjectSchema = createInsertSchema(brandProjects).omit({ id: true, createdAt: true, updatedAt: true });
export const insertMarketResearchSchema = createInsertSchema(marketResearch).omit({ id: true, createdAt: true, updatedAt: true });
export const insertDemandValidationSchema = createInsertSchema(demandValidation).omit({ id: true, createdAt: true, updatedAt: true });
export const insertCompetitorAnalysisSchema = createInsertSchema(competitorAnalysis).omit({ id: true, createdAt: true, updatedAt: true });
export const insertProductOpportunitySchema = createInsertSchema(productOpportunities).omit({ id: true, createdAt: true, updatedAt: true });

// TypeScript types for Brand Builder
export type BrandProject = typeof brandProjects.$inferSelect;
export type InsertBrandProject = z.infer<typeof insertBrandProjectSchema>;
...
export type DemandValidation = typeof demandValidation.$inferSelect;
export type InsertDemandValidation = z.infer<typeof insertDemandValidationSchema>;
